...
parser.add_argument('--exp', type=str, default='cfcmb_1_1', help='model_name')
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                #                                                                                    max_iterations)
                # ====== 置信度阈值区间动态增长 ======
                min_thr = 0.9 * math.log(2)  # ≈0.6238
                max_thr = 1.4 * math.log(2)  # ≈0.9703
                ramp = ramps.sigmoid_rampup(iter_num, max_iterations)
                threshold = min_thr + (max_thr - min_thr) * ramp
                # ================================

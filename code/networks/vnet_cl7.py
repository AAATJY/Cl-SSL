...
class RegionAwareContrastiveLearning(nn.Module):
    def __init__(self, feat_dim=128, temp=0.1, patch_size=16, edge_threshold=0.42,
                 hard_neg_k=32, patch_sample_k=12, edge_patch_sample_k=24, voxel_sample_k=256, voxel_conf_topk=32):
        super().__init__()
        self.temp = temp
        ...
        self.voxel_conf_topk = voxel_conf_topk
    ...
    def forward(self, anchor_feats, positive_feats, labels=None, prob_maps=None,
                enable_patch_level=True, enable_voxel_level=True):
            ...
            if enable_voxel_level:
                for p_idx in sampled_edge_patch_indices:
                    ...
                    voxel_loss += self._rcps_voxel_level_contrast(
                        anchor_patch, positive_patch, label_map=label_map, prob_map=prob_map,
                        voxel_sample_k=self.voxel_sample_k, voxel_conf_topk=self.voxel_conf_topk
                    )

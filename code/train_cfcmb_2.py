...
from utils.cmcfb_2 import AdaptiveMultiScaleRegionContrastiveMemory
...
if __name__ == "__main__":
    ...
    # 替换原有的CFCMB初始化
    feat_dims = {'scale1': 16, 'scale2': 32, 'scale3': 64}  # 根据实际特征维度修改
    amr_cmb = AdaptiveMultiScaleRegionContrastiveMemory(
        feat_dims=feat_dims,
        queue_size=args.queue_size,
        base_tau=0.1,
        device="cuda",
        adaptive_temp=True,
        confidence_threshold=args.base_threshold,
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            student_outputs, multi_scale_features = student_model(
                volume_batch,
            ...
            # 计算多尺度区域中心
            multi_scale_centers = amr_cmb.compute_multi_scale_region_centers(
                multi_scale_features,
                label_batch[:labeled_bs]  # 有标签部分
            )

            # 更新AMR-CMB（仅在有标注处更新）
            amr_cmb.update_from_centers(
                multi_scale_centers,
                scale='high_level',  # 可以循环更新所有尺度
                confidence_scores=None  # 有标签数据置信度高，无需筛选
            )

            # 更新温度因子
            amr_cmb.update_tau_factor(iter_num, max_iterations)

            # 计算有标注对比损失
            Lcon_sup, scale_losses = amr_cmb.multi_scale_contrastive_loss(multi_scale_centers)

            # 更新尺度权重
            amr_cmb.update_scale_weights(scale_losses)

            # 无标注部分的区域中心（基于伪标签）
            unlabeled_multi_scale_centers = amr_cmb.compute_multi_scale_region_centers(
                {k: v[labeled_bs:] for k, v in multi_scale_features.items()},  # 无标签部分
                pseudo_labels  # 伪标签
            )

            # 计算无标注对比损失
            Lcon_unsup, _ = amr_cmb.multi_scale_contrastive_loss(unlabeled_multi_scale_centers)

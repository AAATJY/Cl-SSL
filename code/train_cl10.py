...
import math
from utils.meta_augment_3 import (
...
parser.add_argument('--exp', type=str, default='train_cl10', help='model_name')
...
if __name__ == "__main__":
    ...
    # 新增：仅强度增强的 positive 视图构建（保证与 anchor 完全几何对齐）
    def build_intensity_positive_view(x_tensor, controller):
        """
        x_tensor: torch.FloatTensor [B,1,D,H,W] (已包含空间增强)
        返回：与 x_tensor 几何完全对齐的“强度增强”视图
        """
        augmenter = WeightedWeakAugment(
            AugmentationFactory.get_intensity_only_augs(),
            controller=controller
        )
        x_np = x_tensor.detach().cpu().numpy()  # [B,1,D,H,W]
        out = []
        for i in range(x_np.shape[0]):
            vol_3d = np.squeeze(x_np[i], axis=0)  # [D,H,W]
            sample = {'image': np.ascontiguousarray(vol_3d), 'label': None, 'is_labeled': True}
            aug_sample = augmenter(sample)
            vol = aug_sample['image'][None, ...].astype(np.float32)  # [1,D,H,W]
            out.append(torch.from_numpy(vol))
        out = torch.stack(out, dim=0).to(x_tensor.device)
        return out
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # ================= 数据准备 =================
            # DataLoader 输出（已做基础的空间增强：rot/crop/def）
            weak_volume_batch = sampled_batch['image'].cuda()
            weak_label_batch = sampled_batch['label'].cuda()

            # 额外一次 batch 级强增强（用于分割训练）
            ...
            # ========== 阶段1：教师模型生成伪标签（在“弱视图”上，用于RCPS）==========
            with torch.no_grad():
                teacher_outputs_weak = teacher_model(weak_volume_batch[labeled_bs:], return_contrast_feats=False)
                teacher_outputs_weak = teacher_outputs_weak / args.temperature
                probs_weak = F.softmax(teacher_outputs_weak, dim=1)
                max_probs_weak, _ = torch.max(probs_weak, dim=1)

                # 动态阈值
                threshold = args.base_threshold + (1 - args.base_threshold) * ramps.sigmoid_rampup(iter_num,
                                                                                                   max_iterations)
                mask_weak = (max_probs_weak > threshold).float().unsqueeze(1)  # [B_u,1,D,H,W]
            ...
            # positive：仅强度增强（保证几何对齐）
            with torch.no_grad():
                pos_for_contrast = build_intensity_positive_view(weak_volume_batch, meta_controller)
                _, _, pos_decoder_feats = student_model(pos_for_contrast, return_decoder_feats=True)
                pos_decoder_feats = pos_decoder_feats.detach()
            ...
            # 一致性损失（在“强视图”上对齐教师）
            with torch.no_grad():
                teacher_outputs_strong = teacher_model(volume_batch[labeled_bs:], return_contrast_feats=False)
                teacher_outputs_strong = teacher_outputs_strong / args.temperature
                probs_strong = F.softmax(teacher_outputs_strong, dim=1)
                max_probs_strong, _ = torch.max(probs_strong, dim=1)
                mask_strong = (max_probs_strong > threshold).float().unsqueeze(1)

            consistency_weight = get_current_consistency_weight(iter_num // 150)
            consistency_dist = consistency_criterion(
                student_seg_out[labeled_bs:],
                label_smoothing(probs_strong, factor=0.1)  # 教师标签平滑（与强视图对齐）
            )
            weighted_loss = consistency_dist * mask_strong  # 逐样本加权
            ...
            # ================= 新增：对比学习损失（几何对齐的弱视图 vs 强度增强视图） =================
            contrast_loss = 0
            if contrast_enabled:
                # 早期仅 patch-level，后期逐步打开 voxel-level
                if iter_num < args.contrast_start_iter + 3000:
                    student_model.contrast_learner.loss_weights[0] = 1.0  # patch
                    student_model.contrast_learner.loss_weights[1] = 0.0  # voxel
                else:
                    student_model.contrast_learner.loss_weights[0] = 0.8
                    student_model.contrast_learner.loss_weights[1] = 0.2

                for i in range(weak_volume_batch.size(0)):
                    anchor_feat = weak_decoder_feats[i].unsqueeze(0)  # [1, C, D, H, W]
                    positive_feat = pos_decoder_feats[i].unsqueeze(0)

                    # 保证label_map和prob_map为5维 [B, 1, D, H, W]，且与弱视图对齐
                    if i < labeled_bs:
                        label_map = weak_label_batch[i].unsqueeze(0).unsqueeze(1)  # [1, 1, D, H, W]
                        prob_map = None
                    else:
                        pseudo_label = torch.argmax(probs_weak[i - labeled_bs], dim=0).unsqueeze(0).unsqueeze(1)  # [1,1,D,H,W]
                        prob_map = max_probs_weak[i - labeled_bs].unsqueeze(0).unsqueeze(1)  # [1,1,D,H,W]
                ...
                contrast_loss = contrast_loss / weak_volume_batch.size(0)
                # 更平滑的 warmup（5k iter），且降低上限以避免主损失被扰动
                warm = max(0.0, min(1.0, (iter_num - args.contrast_start_iter) / 5000.0))
                contrast_weight = min(0.05, args.contrast_weight) * warm
                weighted_contrast_loss = contrast_weight * contrast_loss
            else:
                weighted_contrast_loss = 0.0
            ...
            iter_num = iter_num + 1
            writer.add_scalar('uncertainty/mask_per', torch.sum(mask_strong) / mask_strong.numel(), iter_num)
